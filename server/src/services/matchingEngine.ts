...
export function calculateHlaScore(donorHla?: HlaTyping | null, recipientHla?: HlaTyping | null): number {
  if (!donorHla || !recipientHla) return 80.0;
...
export function calculateTransitTimeMinutes(distanceKm: number): number {
  if (distanceKm <= 50) return Math.round(25 + distanceKm * 0.8);
  if (distanceKm <= 300) return Math.round(40 + distanceKm * 0.5);
...
export function calculateUrgencyScore(urgencyLevel: string, registeredAt?: Date | null): number {
...
export function calculateMatchScore(params: {
  ...
  recipientHla?: HlaTyping | null;
  donorLat?: number | null;
  donorLon?: number | null;
  recipientLat?: number | null;
  recipientLon?: number | null;
  urgencyLevel: string;
  registeredAt?: Date | null;
  coldIschemiaHours?: number;
}): { compatible: boolean; breakdown: MatchScoreBreakdown; distanceKm: number; transitMinutes: number } {
  ...
  const composite = Math.round(
    (bloodScore * 0.35 + hlaScore * 0.25 + distanceScore * 0.20 + urgencyScore * 0.20) * 100
  ) / 100;
